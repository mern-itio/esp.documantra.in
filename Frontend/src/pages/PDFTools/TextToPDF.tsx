...
export const TextToPdf: React.FC = () => {
  ...
  const toolInfo = {
    name: 'Text to PDF',
    description: 'Convert Text files to PDF file with high accuracy',
    icon: FileTextIcon,
    premium: false,
    // badge: 'Popular',
 ...
 const handleFileUpload = (event: React.ChangeEvent<HTMLInputElement>) => {
  ...
  const textFiles = files.filter(
    (file) => file.type === "application/txt" || file.name.toLowerCase().endsWith(".txt")
  );
    ...
    try {
      ...
      for (let i = 0; i < uploadedFiles.length; i++) {
        ...
        try {
          ...
          const result = await pdfService.convertTxtToPdf(file);
          ...
          {uploadedFiles.length > 0 && (
            <div className="bg-white rounded-xl border border-gray-200 p-6">
              <div className="flex items-center justify-between mb-4">
                <h3 className="text-lg font-semibold text-gray-900">Convert Text files to PDF</h3>
                <button
