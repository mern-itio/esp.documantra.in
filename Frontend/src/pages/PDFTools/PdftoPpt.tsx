...
export const PdftoPpt: React.FC = () => {
  ...
  const toolInfo = {
    name: 'PDF to PPT',
    description: 'Convert PDF Files to Powerpoint Presentation with high accuracy',
    icon: FileTextIcon,
    premium: false,
    // badge: 'Popular',
  ...
  const processFiles = async () => {
    ...
    try {
      ...
      for (let i = 0; i < uploadedFiles.length; i++) {
        ...
        try {
          console.log(`Converting ${file.name} to PPT...`);
          
          // Call the conversion API using the service
          const result = await pdfService.convertPdfToPpt(file);
          ...
          {uploadedFiles.length > 0 && (
            <div className="bg-muted rounded-xl border border-border p-6">
              <div className="flex items-center justify-between mb-4">
                <h3 className="text-lg font-semibold text-foreground">Convert PDF to PPT</h3>
                <button
