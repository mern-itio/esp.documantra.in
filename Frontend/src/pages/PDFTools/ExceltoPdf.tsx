...
export const ExcelToPdf: React.FC = () => {
 ...
 const handleFileUpload = (event: React.ChangeEvent<HTMLInputElement>) => {
  ...
  const excelFiles = files.filter(
    (file) => file.type === "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet" || file.name.toLowerCase().endsWith(".xlsx")
  );
  if (excelFiles.length > 0) {
    ...
    setUploadTimes((prev) => [...prev, ...excelFiles.map(() => now)]);
  }
};
    ...
    try {
      ...
      for (let i = 0; i < uploadedFiles.length; i++) {
        ...
        try {
          ...
          const result = await pdfService.convertExcelToPdf(file);
            ...
            {uploadedFiles.length === 0 && (
              ...
              >
                ...
                <p className="text-muted-foreground mb-4">
                  Supports: {toolInfo.inputFormats?.join(', ') || 'Excel files'}
                </p>
                <button
          ...
          {uploadedFiles.length > 0 && (
            <div className="bg-muted rounded-xl border border-border p-6">
              <div className="flex items-center justify-between mb-4">
                <h3 className="text-lg font-semibold text-foreground">Convert Excel to PDF</h3>
                <button
