...
export const PptToPdf: React.FC = () => {
  ...
  const toolInfo = {
    name: 'PPT to PDF',
    description: 'Convert Powerpoint files to PDF with high accuracy',
    icon: FileTextIcon,
    premium: false,
    // badge: 'Popular',
 ...
 const handleFileUpload = (event: React.ChangeEvent<HTMLInputElement>) => {
  ...
  // Allow only PPT files
  const pptFiles = files.filter(
    (file) => file.type === "application/vnd.openxmlformats-officedocument.presentationml.presentation" || file.name.toLowerCase().endsWith(".pptx")
  );
  if (pptFiles.length > 0) {
    ...
    const now = new Date();
    setUploadedFiles((prev) => [...prev, ...pptFiles]);
    setUploadTimes((prev) => [...prev, ...pptFiles.map(() => now)]);
  }
};
    ...
    try {
      ...
      for (let i = 0; i < uploadedFiles.length; i++) {
        ...
        try {
          ...
          const result = await pdfService.convertPptToPdf(file);
            ...
            {uploadedFiles.length === 0 && (
              ...
              >
                ...
                <p className="text-muted-foreground mb-4">
                  Supports: {toolInfo.inputFormats?.join(', ') || 'PPT files'}
                </p>
                <button
          ...
          {uploadedFiles.length > 0 && (
            <div className="bg-muted rounded-xl border border-border p-6">
              <div className="flex items-center justify-between mb-4">
                <h3 className="text-lg font-semibold text-foreground">Convert PPT to PDF</h3>
                <button
              ...
              {isProcessing && (
                <div className="mb-4">
                  <div className="flex items-center justify-between mb-2">
                    <span className="text-sm font-medium text-foreground">Converting PPT File...</span>
