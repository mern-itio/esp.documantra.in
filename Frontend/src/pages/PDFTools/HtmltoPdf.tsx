...
export const HtmlToPdf: React.FC = () => {
  ...
  const toolInfo = {
    name: 'HTML to PDF',
    description: 'Convert HTML files to PDF file with high accuracy',
    icon: FileTextIcon,
    premium: false,
    // badge: 'Popular',
 ...
 const handleFileUpload = (event: React.ChangeEvent<HTMLInputElement>) => {
  ...
  const htmlFiles = files.filter(
    (file) => file.type === "application/html" || file.name.toLowerCase().endsWith(".html")
  );
  if (htmlFiles.length > 0) {
    ...
    setUploadedFiles((prev) => [...prev, ...htmlFiles]);
  }
};
    ...
    try {
      ...
      for (let i = 0; i < uploadedFiles.length; i++) {
        ...
        try {
          ...
          const result = await pdfService.convertHtmlToPdf(file);
            ...
            {uploadedFiles.length === 0 && (
              ...
              >
                ...
                <p className="text-muted-foreground mb-4">
                  Supports: {toolInfo.inputFormats?.join(', ') || 'HTML files'}
                </p>
                <button
          ...
          {uploadedFiles.length > 0 && (
            <div className="bg-muted rounded-xl border border-border p-6">
              <div className="flex items-center justify-between mb-4">
                <h3 className="text-lg font-semibold text-foreground">Convert HTML files to PDF Files</h3>
                <button
