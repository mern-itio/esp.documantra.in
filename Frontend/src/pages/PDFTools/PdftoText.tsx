...
export const PdftoText: React.FC = () => {
  ...
  const toolInfo = {
    name: 'PDF to Text',
    description: 'Convert PDF files to Text file with high accuracy',
    icon: FileTextIcon,
    premium: false,
    // badge: 'Popular',
  ...
  const processFiles = async () => {
    ...
    try {
      ...
      for (let i = 0; i < uploadedFiles.length; i++) {
        ...
        try {
          console.log(`Converting ${file.name} to Text file...`);
          
          // Call the conversion API using the service
          const result = await pdfService.convertPdfToTxt(file);
          ...
          {uploadedFiles.length > 0 && (
            <div className="bg-white rounded-xl border border-gray-200 p-6">
              <div className="flex items-center justify-between mb-4">
                <h3 className="text-lg font-semibold text-gray-900">Convert PDF to Text file</h3>
                <button
