...
export const DoctoPdf: React.FC = () => {
  ...
  const toolInfo = {
    name: 'Word to PDF',
    description: 'Convert Word documents to PDF documents with high accuracy',
    icon: FileTextIcon,
    premium: false,
    // badge: 'Popular',
 ...
 const handleFileUpload = (event: React.ChangeEvent<HTMLInputElement>) => {
  ...
  // Filter to allow only DOC and DOCX files
  ...
  if (docFiles.length > 0) {
    ...
    setUploadTimes(prev => [...prev, ...docFiles.map(() => now)]);
  }
};
  ...
  const handleDrop = (event: React.DragEvent) => {
    ...
    // Filter to only allow DOC and DOCX files
    const docFiles = files.filter(
    file =>
      file.type ===
        'application/vnd.openxmlformats-officedocument.wordprocessingml.document' || // .docx
  ...
  const removeFile = (index: number) => {
    ...
    if (newUploaded.length === 0) {
      // Clear results and progress when no files remain
      setResults([]);
      setProcessingProgress(0);
      setIsProcessing(false);
    }
    ...
    try {
      ...
      for (let i = 0; i < uploadedFiles.length; i++) {
        ...
        try {
          ...
          const result = await pdfService.convertDocToPdf(file);
            ...
            {uploadedFiles.length === 0 && (
              ...
              >
                ...
                <p className="text-muted-foreground mb-4">
                  Supports: {toolInfo.inputFormats?.join(', ') || 'Word files'}
                </p>
                <button
          ...
          {uploadedFiles.length > 0 && (
            <div className="bg-muted rounded-xl border border-border p-6">
              <div className="flex items-center justify-between mb-4">
                <h3 className="text-lg font-semibold text-foreground">Convert Word to PDF</h3>
                <button
              ...
              {isProcessing && (
                <div className="mb-4">
                  <div className="flex items-center justify-between mb-2">
                    <span className="text-sm font-medium text-foreground">Converting Doc File...</span>
