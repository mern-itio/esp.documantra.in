...
def add_annotation(document_id, annotation, page_number):
    try:
        ...
        # Save the document - always save to a new file to avoid incremental issues
        timestamp = int(datetime.now().timestamp() * 1000)
        new_filename = f"editor-{document_id}_{timestamp}.pdf"
        ...
        try:
            ...
            import shutil
            shutil.move(new_path, doc_path)
        except Exception as save_error:
            print(f"Error saving document: {save_error}")
            # If save fails, try without any special options
            try:
                doc.save(new_path, garbage=0)
                import shutil
                shutil.move(new_path, doc_path)
...
if __name__ == "__main__":
    document_id = "doc_1756985442154"
    annotation = {"id":"ann_1756985452425_7mogh3187","type":"text","pageNumber":1,"position":{"x":62.83746337890625,"y":40.19999694824219,"width":100,"height":20},"content":"Click to edit text","style":{"color":"#000000","fontSize":12,"fontFamily":"Arial","fontWeight":"normal","textAlign":"left","backgroundColor":"transparent"},"author":"Current User","createdAt":"2025-09-04T11:30:52.425Z","isLocked":False,"isVisible":True,"zIndex":1}
    page_number = 1
